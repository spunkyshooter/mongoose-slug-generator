...
module.exports = function (schema, options) {
    ...
    schema.eachPath(function (pathname, schemaType) {
        if (schemaType.instance == "String" && schemaType.options && schemaType.options.slug) {
            ...
            if(typeof schemaType.options.formatter === 'function'){
                slug.formatter = schemaType.options.formatter;
            }else {
                slug.formatter =  function(values){ return values; }
            }

            if(typeof schemaType.options.filter === 'function'){
                slug.filter = schemaType.options.filter;
            }else{
                slug.filter = function(doc){ return true; }
            }
    ...
    schema.pre('save', function (next) {
        ...
        async.each(slugs, function (item, callback) {
            ...
            values = item.formatter(values);

            if(!item.filter(doc)){
                callback()
            }
            else if (!(item.unique || item.unique_slug)) {
